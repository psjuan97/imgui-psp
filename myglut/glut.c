...
void glutInitDisplayMode(unsigned int mode) { glut_display_mode = mode; }
...
static EGLint attrib_list[] = {EGL_RED_SIZE,     8, /* 0 */
                               EGL_GREEN_SIZE,   8, /* 2 */
                               EGL_BLUE_SIZE,    8, /* 4 */
                               EGL_ALPHA_SIZE,   0, /* 6 */
...
static void cleanup(void) { EGLCHK(eglTerminate(dpy)); }
...
int glutCreateWindow(const char *title) {
  ...
  if (num_configs == 0) {
    __pspgl_log("glutCreateWindow: eglChooseConfig returned no configurations "
                "for display mode %x\n",
                glut_display_mode);
    return 0;
  }
...
static const unsigned long keycode[] = {
...
void glutMainLoop(void) {
  ...
  do {
    ...
    if (glut_joystick_func) {
      ...
      glut_joystick_func(pad.Buttons, (pad.Lx * 2000L) / 256 - 1000,
...
void glutPostRedisplay(void) { glut_redisplay_posted = 1; }

void glutSwapBuffers(void) { EGLCHK(eglSwapBuffers(dpy, surface)); }

void glutDisplayFunc(void (*func)(void)) { glut_display_func = func; }
...
void glutMotionFunc(void (*func)(int x, int y)) { glut_motion_func = func; }

void glutPassiveMotionFunc(void (*func)(int x, int y)) {
  glut_passive_motion_func = func;
}

void glutIdleFunc(void (*func)(void)) { glut_idle_func = func; }
